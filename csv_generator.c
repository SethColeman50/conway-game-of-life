...
int main(int argc, const char* argv[]) {
    ...
    for (size_t t = 0; t < num_steps; t++) {
        ...
        // save the output matrix with step number in the filename
        char step_filename[100];
        sprintf(step_filename, "gif_materials/%s_step_%zu.csv", output_name, t+1);
        printf("Saving step %zu to %s\n", t+1, step_filename);
        output.to_csv(step_filename);
    }
